...
import { TrashIcon, PlusIcon, MinusIcon, ExclamationCircleIcon, CalendarIcon } from '@heroicons/react/20/solid';
...
export const CartSidebar: React.FC<CartSidebarProps> = ({ items, onUpdateQuantity, onRemove, onAction, isAttendanceMode = false }) => {
    ...
    return (
        ...
        >
            <header className="p-4 border-b border-gray-100 bg-gray-50/50 rounded-t-lg">
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-3" role="list">
                {items.length === 0 ? (
                    ...
                    items.map(item => (
                        <article
                            key={item.id}
                            className="group relative bg-white border border-gray-100 rounded-xl p-3 hover:border-indigo-100 hover:shadow-sm transition-all"
                            role="listitem"
                        >
                            ...
                            {(item.anticipationFee || 0) > 0 && (
                                <div className="flex items-center gap-2 text-xs text-orange-700 bg-orange-50 px-2 py-1 rounded mb-3 border border-orange-100">
                            ...
                            {(item.limitFee || 0) > 0 && (
                                <div className="flex items-center gap-2 text-xs text-red-700 bg-red-50 px-2 py-1 rounded mb-3 border border-red-100">
                                    <ExclamationCircleIcon className="w-3 h-3 flex-shrink-0" />
            ...
            <footer className="p-4 border-t border-gray-100 bg-gray-50/80 mt-auto rounded-b-lg space-y-3">
